...
import conexp.fx.core.algorithm.IFox;
...
public final class FCAInstance<G, M> {
  ...
  public final ObservableList<Implication<M>> implications     =
                                                                   FXCollections
  ...
  public final void simpleInit(final InitLevel lvl) {
    ...
    if (lvl.isOrNeedsLevel(InitLevel.CONCEPTS)) {
      executor.submit(NextExtent.concepts(lattice));
      ...
      if (lvl.isOrNeedsLevel(InitLevel.LAYOUT))
        executor.submit(GeneticLayouter.seeds(layout, false, Constants.GENERATIONS, Constants.POPULATION, tab == null
            ? false : tab.threeDimensions(), conflictDistance, tpe));
    }
  }
  ...
  public final void storeToFile() {
    executor.submit(new BlockingTask("Store") {
      ...
      protected final void _call() {
        ...
        case CXT:
          CXTExporter.export(context, tab == null ? null : tab.rowMap(), tab == null ? null : tab.colMap(), file);
          break;
        case CFX:
          CFXExporter.export(
              context,
  ...
  public final void setTabConfiguration(final TabConfiguration tabConfiguration) {
