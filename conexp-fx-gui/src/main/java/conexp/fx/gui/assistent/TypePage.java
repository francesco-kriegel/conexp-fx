  ...
  extends AssistentPage<Request<?, ?>>
{
      ...
      final Button choose = ButtonBuilder.create().text("Choose File...").onAction(new EventHandler<ActionEvent>()
        {
          ...
          {
            fileProperty.set(conExp.showOpenFileDialog(title, fileFormats));
          }
        }).build();
  ...
  private final GUI conExp;
  private final Type              type;
  private final Accordion         accordion = new Accordion();

  public TypePage(final GUI conExp, final Type type)
  {
...
//              return new BinaryRelationsFromInt(nSpinner.getValue());
  ...
  private final SourcePane orderPane()
  {
    ...
    final ChoiceBox<MatrixContext<?, ?>> orderChoice =
        ChoiceBoxBuilder.<MatrixContext<?, ?>> create().minWidth(320).maxWidth(320).items(this.conExp.orders).build();
  ...
  private final SourcePane contextPane()
  {
    ...
    final ChoiceBox<MatrixContext<?, ?>> contextChoice =
        ChoiceBoxBuilder.<MatrixContext<?, ?>> create().minWidth(320).maxWidth(320).items(conExp.contexts).build();
...
//              return new Approximation(contextChoice.valueProperty().get(), null, null);
  ...
  {
    conExp.newTab(resultProperty.get());
  }
}
