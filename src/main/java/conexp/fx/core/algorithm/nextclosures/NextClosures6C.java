...
public final class NextClosures6C {
  ...
  public static final <G, M> ResultC<G, M> compute(
    ...
    final ResultC<G, M> result = compute(
        cxt,
        clop,
        verbose,
        tpe);
  ...
  public static final <G, M> ResultC<G, M> computeWithBackgroundImplications(
    ...
    return compute(
        cxt,
        ClosureOperator.fromImplicationSet(backgroundImplications),
        verbose);
  }

  public static final <G, M> ResultC<G, M> computeIceberg(
      final Context<G, M> cxt,
      final int minSupp,
      final boolean verbose) {
    return compute(
        cxt,
        ClosureOperator.byMinimalSupport(
            minSupp,
            cxt),
        verbose);
  }

  public static final <G, M> ResultC<G, M> computeByMaxCard(
      final Context<G, M> cxt,
      final int maxCard,
      final boolean verbose) {
    return compute(
        cxt,
        ClosureOperator.byMaximalCardinality(
            maxCard,
            cxt.colHeads()),
        verbose);
  }

  public static final <G, M> ResultC<G, M> computeBelow(
      final Context<G, M> cxt,
      final Collection<M> elements,
      final boolean verbose) {
    return compute(
        cxt,
        ClosureOperator.isSubsetOf(
            elements,
            cxt.colHeads()),
        verbose);
  }

  public static final <G, M> ResultC<G, M> computeAbove(
      final Context<G, M> cxt,
      final Collection<M> elements,
      final boolean verbose) {
    return compute(
        cxt,
        ClosureOperator.containsAllFrom(
            elements,
            cxt.colHeads()),
        verbose);
  }
