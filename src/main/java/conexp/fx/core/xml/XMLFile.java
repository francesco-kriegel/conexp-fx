...
/**
 * Use an XMLFile for persistent configuration values. Each value is adressed by
 * a structured key. Keys must be lower-case strings (!!!) and are simply
 * structured by dots, like URLs or names within ontologies.
 */
public final class XMLFile extends AbstractCompoundData {
	...
	public XMLFile(final File file) throws IOException {
		super(Datatype.DOCUMENT, "", readElementFromFile(file), new Metadata(
				readElementFromFile(file)));
		this.file = file;
	}

	private synchronized static final Element readElementFromFile(
	...
	private synchronized final void writeDataToElement(
			final Map<String, Data<?>> map, final Element element,
		...
		for (final String key : Collections3.sort(map.keySet())) {
			...
			if (metadata != null) {
				...
				metadata_data.appendElement("type").text(
				...
				case STRING_LIST:
				case COMPOUND_LIST:
					JsoupUtil.firstOrAppendElement(metadata_data, "metadata",
							"subkey").text(value.toListData().getSubkey());
				default:
				}
			}
			final Element data = JsoupUtil.firstOrAppendElement(element,
					Key.toArray(key));
			switch (value.getType()) {
			case BOOLEAN:
			...
			case COMPOUND:
				writeDataToElement(
						...
						data,
						metadata != null ? JsoupUtil.firstOrAppendElement(
								metadata_data, "metadata") : null);
				break;
			case BOOLEAN_LIST:
				for (Boolean boolean_ : value.toBooleanListData())
					data.appendElement(value.toListData().getSubkey()).text(
							boolean_.toString());
				break;
			case INTEGER_LIST:
			...
			case COMPOUND_LIST:
				...
				for (Map<String, Data<?>> map_ : value.toCompoundListData())
					writeDataToElement(
							...
							i++ == 0 && metadata != null ? JsoupUtil
									.firstOrAppendElement(metadata_data,
											"metadata") : null);
			case METADATA:
			default:
			}
	...
	private synchronized final void writeDocumentToFile(final Document document)
			throws IOException {
	...
	public static final void createEmptyConfiguration(File file)
			throws IOException {
