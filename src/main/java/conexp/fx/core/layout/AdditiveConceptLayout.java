...
public final class AdditiveConceptLayout<G, M> extends ConceptLayout<G, M> {
  ...
  public final Map<G, Point3D>           _seedsG      = new ConcurrentHashMap<G, Point3D>();
  public final ObservableMap<G, Point3D> seedsG       = FXCollections.observableMap(_seedsG);
  public final Map<G, Point3D>           seedHistoryG = new ConcurrentHashMap<G, Point3D>();

  public final Map<M, Point3D>           _seedsM      = new ConcurrentHashMap<M, Point3D>();
  public final ObservableMap<M, Point3D> seedsM       = FXCollections.observableMap(_seedsM);
  ...
  public AdditiveConceptLayout(
      ...
      final Map<G, Point3D> initialSeedsG,
      final Map<M, Point3D> initialSeedsM,
      final Type type) {
...
//      positionBindings.put(concept, 
    return new ObjectBinding<Point3D>() {
      ...
      {
        if (observe)
          bind(seedsG, concept.extent(), seedsM, concept.intent());
        else
          bind(seedsG, seedsM);
      }
      ...
      public void dispose() {
        if (observe)
          unbind(seedsG, concept.extent(), seedsM, concept.intent());
        else
          unbind(seedsG, seedsM);
        super.dispose();
      }
...
//      })
  ...
  public final boolean updateSeeds(final Map<G, Point3D> seedUpdatesG, final Map<M, Point3D> seedUpdatesM) {
    boolean ret = false;
    if (!_seedsG.equals(seedUpdatesG))
      synchronized (seedsG) {
        ret = true;
        seedsG.clear();
    ...
    if (!_seedsM.equals(seedUpdatesM))
      synchronized (seedsM) {
        ret = true;
        seedsM.clear();
      ...
      synchronized (seedsM) {
        ...
        case INTENT_CHAIN_SEEDS:
          ...
          case INTENT_SEEDS:
            ...
            break;
          case INTENT_CHAIN_SEEDS:
            final double size = affectedSeedsG.size() + affectedSeedsM.size();
...
//      seedsM.putAll(new HashMap<M, Point3D>(Maps.transformValues(seedsM, Points.XY_PROJECTION)));
  ...
  public final AdditiveConceptLayout<G, M> clone() {
    return new AdditiveConceptLayout<G, M>(lattice, seedsG, seedsM, type);
  }
