  ...
  protected final class ConceptVertex extends Vertex {
    ...
    private final class DragHandler implements EventHandler<MouseEvent> {
      ...
      private final void drag(final MouseEvent event) {
        layout.move(element, movement, getTarget(event).subtract(position.getValue()));
      }
...
//        System.out.println(layout.getPosition(element));
        event.consume();
        if (!toolBar.highlight.isSelected() || controller.isDragging())
          return;
        if (event.getEventType().equals(MouseEvent.MOUSE_ENTERED)) {
          highlight();
          if (controlBox.attributeLabelText.getSelectionModel().getSelectedItem().equals(AttributeLabelText.NONE)) {
            final Consumer<Concept<G, M>> f = c -> {
              synchronized (layout.lattice.attributeConcepts) {
                for (Entry<M, Concept<G, M>> e : layout.lattice.attributeConcepts.entrySet())
                  if (e.getValue().equals(c)) {
                    new AttributeLabel(e.getKey(), c);
                    break;
                  }
              }
            };
            ...
            case UPPER_NEIGHBORS:
              f.accept(element);
              layout.lattice.upperNeighbors(element).forEach(f::accept);
              break;
            case LOWER_NEIGHBORS:
              f.accept(element);
              layout.lattice.lowerNeighbors(element).forEach(f::accept);
              break;
            case NEIGHBORS:
              f.accept(element);
              layout.lattice.upperNeighbors(element).forEach(f::accept);
            ...
            case FILTER:
              f.accept(element);
              layout.lattice.filter(element).forEach(f::accept);
              break;
            case IDEAL:
              f.accept(element);
              layout.lattice.ideal(element).forEach(f::accept);
              break;
            case FILTER_IDEAL:
              f.accept(element);
              layout.lattice.filter(element).forEach(f::accept);
          ...
          if (controlBox.objectLabelText.getSelectionModel().getSelectedItem().equals(ObjectLabelText.NONE)) {
            final Consumer<Concept<G, M>> f = c -> {
              synchronized (layout.lattice.objectConcepts) {
                for (Entry<G, Concept<G, M>> e : layout.lattice.objectConcepts.entrySet())
                  if (e.getValue().equals(c)) {
                    new ObjectLabel(e.getKey(), c);
                    break;
                  }
              }
            };
    ...
    private ConceptVertex(final Concept<G, M> concept, final Double layoutX, final Double layoutY) {
      super(concept, new Circle(), layout.getOrAddPosition(concept), layoutX, layoutY);
      init();
    ...
    protected final void init() {
      node.radiusProperty().bind(new DoubleBinding() {
        ...
        public final double computeValue() {
          return controlBox.vertexRadius.getSelectionModel().selectedItemProperty().get().get(
              fca.context,
              fca.lattice,
              element);
        }
...
//        l.content.toFront();
    ...
    private ConceptEdge(final Pair<Concept<G, M>, Concept<G, M>> concepts) {
      ...
      line.strokeWidthProperty().bind(new DoubleBinding() {
        ...
        protected final double computeValue() {
          return controlBox.edgeStroke.getSelectionModel().selectedItemProperty().get().get(
              fca.context,
              fca.lattice,
              concepts);
        }
  ...
  protected final class ObjectLabel extends LowerLabel {
    ...
    private ObjectLabel(final G object, final Concept<G, M> concept) {
      super(new ObjectBinding<Concept<G, M>>() {
        ...
        {
          layout.lattice.objectConcepts.addListener(new MapChangeListener<G, Concept<G, M>>() {
      ...
      synchronized (objectLabels) {
        final ObjectLabel old = objectLabels.put(object, this);
        if (old != null)
          old.dispose();
      }
    }
    ...
    private ObjectLabel(final Concept<G, M> concept) {
      super(new SimpleObjectProperty<Concept<G, M>>(concept), new StringBinding() {
        ...
        protected final String computeValue() {
          return controlBox.objectLabelText.getSelectionModel().selectedItemProperty().get().get(
              fca.context,
              fca.lattice,
              concept);
        }
  ...
  protected final class AttributeLabel extends UpperLabel {
    ...
    private AttributeLabel(final M attribute, final Concept<G, M> concept) {
      super(new ObjectBinding<Concept<G, M>>() {
        ...
        {
          layout.lattice.attributeConcepts.addListener(new MapChangeListener<M, Concept<G, M>>() {
      ...
      text.styleProperty().bind(controlBox.textSizeBinding);
      synchronized (attributeLabels) {
        final AttributeLabel old = attributeLabels.put(attribute, this);
        if (old != null)
          old.dispose();
      }
    }
    ...
    private AttributeLabel(final Concept<G, M> concept) {
      super(new SimpleObjectProperty<Concept<G, M>>(concept), new StringBinding() {
        ...
        protected final String computeValue() {
          return controlBox.attributeLabelText.getSelectionModel().selectedItemProperty().get().get(
              fca.context,
              fca.lattice,
              concept);
        }
  ...
  private final class CFXControlBox {
    ...
    private final ChoiceBox<AdditiveConceptLayout.Type> layoutType         =
        FXControls.newChoiceBox(AdditiveConceptLayout.Type.HYBRID, AdditiveConceptLayout.Type.values());
    ...
    private final ChoiceBox<ObjectLabelText>            objectLabelText    =
        FXControls.newChoiceBox(ObjectLabelText.OBJECT_LABELS, ObjectLabelText.values());
    ...
    private final void createContent() {
      ...
      content.getChildren().add(new Text("Layout Type:"));
      content.getChildren().add(layoutType);
      content.getChildren().add(RectangleBuilder.create().height(inset).build());
    ...
    private final void createListeners() {
      ...
      // TODO
//      layout.bindType(layoutType.valueProperty());
      ...
      attributeLabelText.getSelectionModel().selectedItemProperty().addListener(
          new ChangeListener<AttributeLabelText>() {
            ...
            public final void changed(
              ...
              case SEED_LABELS:
                ...
                synchronized (layout.lattice.attributeConcepts) {
                  for (Entry<M, Concept<G, M>> e : layout.lattice.attributeConcepts.entrySet())
                    // TODO
                    // if (layout.seedsM.containsKey(e.getKey()))
                    new AttributeLabel(e.getKey(), e.getValue());
                }
                break;
              case INTENT_SIZE:
      ...
      hideBottom.selectedProperty().addListener(new ChangeListener<Boolean>() {
        ...
        public void
          ...
          layout.invalidate();
          if (hide) {
//                vertices.get(layout.bottomConcept.get()).node.opacityProperty().bind(zero);
...
//                vertices.get(layout.bottomConcept.get()).node.opacityProperty().unbind();
//                vertices.get(layout.bottomConcept.get()).node.opacityProperty().set(1d);
...
//      transformationCircularButton.setMinHeight(24);
      transformationBox
          .getChildren()
          .addAll(transformation2DButton, transformation3DButton, transformationXYButton, transformationPolarButton
//          ,transformationCircularButton
      );
...
//      });
  ...
  public ConceptGraph(final FCADataset<G, M> fcaInstance, final ConceptLayout<G, M, ?> layout) {
    super(layout);
    ...
    this.textSize.bind(this.controlBox.labelTextSize.valueProperty());
  }
  ...
  private final void initLayoutListeners() {
    ...
    initEdgeListeners();
    initLabelListeners();
    layout.lattice.addEventHandler(new RelationEventHandler<Concept<G, M>, Concept<G, M>>() {
...
//        synchronized (layout.generators) {
        for (final Concept<G, M> concept : event.getRows())
          if (layout.generators.containsKey(concept)) {
            synchronized (layout.generators) {
              final Concept<G, M> generator = layout.generators.remove(concept);
...
//        }
  ...
  private final void initLabelListeners() {
    ...
    layout.lattice.attributeConcepts.addListener(new MapChangeListener<M, Concept<G, M>>() {
      ...
      public final void onChanged(final MapChangeListener.Change<? extends M, ? extends Concept<G, M>> change) {
        if (!controlBox.attributeLabelText.getSelectionModel().getSelectedItem().equals(
    ...
    front.addEventHandler(ScrollEvent.SCROLL, new EventHandler<ScrollEvent>() {
      ...
      public final void handle(final ScrollEvent event) {
        if (transformation.get().equals(GraphTransformation.GRAPH_3D))
          layout.rotate(event.getDeltaX() / (80d * Math.PI));
//          layout.rotate((event.getDeltaX() == 0 ? event.getDeltaY() : event.getDeltaX()) / (80d * Math.PI));
      }
    });
      ...
      public final void handle(final MouseEvent event) {
        ...
        if (event.getEventType().equals(MouseEvent.MOUSE_PRESSED)) {
          ...
          if (path != null && path.getElements().size() > 1) {
            ...
            final Concept<G, M> supremum = layout.lattice.supremum(concepts);
            final Concept<G, M> infimum = layout.lattice.infimum(concepts);
            highlight(
                true,
                Iterables.concat(
  ...
  protected final BoundingBox getContentBoundingBox() {
    if (fca == null || layout == null || controlBox == null)
      return new BoundingBox(0, 0, 0, 0, 0, 0);
    return layout.getCurrentBoundingBox(controlBox.hideBottom.isSelected(), controlBox.hideTop.isSelected());
  }
...
//      inner.addAll(Collections3.intersection(lower, upper));
    ...
    public final Iterable<HighlightRequest> upperNeighbors(final Concept<G, M> concept) {
      ...
      return Collections.<HighlightRequest> singleton(
          new HighlightRequest(
              layout.lattice.upperNeighbors(concept),
              EdgeHighlight.CONTAINS_LOWER,
              COLOR_UPPER,
              COLOR_UPPER,
              COLOR_LABEL_DEFAULT,
    ...
    public final Iterable<HighlightRequest> lowerNeighbors(final Concept<G, M> concept) {
      ...
      return Collections.<HighlightRequest> singleton(
          new HighlightRequest(
              layout.lattice.lowerNeighbors(concept),
              EdgeHighlight.CONTAINS_UPPER,
              COLOR_LOWER,
              COLOR_LOWER,
              COLOR_LOWER,
    ...
    public final Iterable<HighlightRequest> strictFilter(final Concept<G, M> concept) {
      ...
      return Collections.<HighlightRequest> singleton(
          new HighlightRequest(
              Sets.difference(layout.lattice.filter(concept), Collections.singleton(concept)),
              EdgeHighlight.CONTAINS_LOWER,
              COLOR_UPPER,
              COLOR_UPPER,
              COLOR_LABEL_DEFAULT,
    ...
    public final Iterable<HighlightRequest> strictIdeal(final Concept<G, M> concept) {
      ...
      return Collections.<HighlightRequest> singleton(
          new HighlightRequest(
              Sets.difference(layout.lattice.ideal(concept), Collections.singleton(concept)),
              EdgeHighlight.CONTAINS_UPPER,
              COLOR_LOWER,
              COLOR_LOWER,
              COLOR_LOWER,
    ...
    public final Iterable<HighlightRequest>
      ...
      return Collections.<HighlightRequest> singleton(
          new HighlightRequest(
              layout.lattice.interval(lowerConcept, upperConcept),
              EdgeHighlight.CONTAINS_BOTH,
              COLOR_INTERVAL,
              COLOR_INTERVAL,
              COLOR_INTERVAL,
